...
|-----------|------------|---------|
| AI Runtime | Ollama | Local AI model server |
| Vision Model | Moondream | Lightweight vision-language model |
...
2. The image URL is sent to the local Ollama server running the Moondream vision model
...
For the AI tag generation to function, the following must be running:

1. **Ollama Server**: Start with command `ollama serve`
2. **Moondream Model**: Install with command `ollama pull moondream`
